...
`codex-id` 是一个轻量的 Codex 账号切换 CLI，命令名为 `cid`。

它把账号统一存放在 `~/.codex/codex-id.json`，切换时再把选中账号的
...
- `~/.codex/` 可写，`~/.codex/codex-id.json` 不是符号链接
...
- 查询额度前会检查账号库里的 `access_token`，已过期或 1 天内到期时，
  自动用 `refresh_token` 续期并回写 `~/.codex/codex-id.json`
- 如果续期的是当前生效账号，会同步更新 `~/.codex/auth.json`
- 续期失败不会伪装成功；后续探活仍会按 Usage 接口结果显示离线或未知
- 优先显示套餐、Next 短周期额度、Total 总周期额度
- 账号列直接显示完整邮箱；邮箱缺失时显示短 ID，不做截断
- 账号显示顺序与 `cid use N`、`cid remove N` 的编号解析顺序一致
...
- 付费账号比较 Next 剩余额度；只要 Total 剩余额度为 0，就视为冷却账号，不参与自动切换
- 免费账号没有 Next 额度，只比较 Total 剩余额度
